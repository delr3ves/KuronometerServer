...
object PlatformSerializer extends JsonSerializer[Platform] {
  override def serialize(
      value: Platform,
      gen: JsonGenerator,
      serializers: SerializerProvider): Unit = gen.writeString(value.name)
}

object BuildToolDeserializer extends JsonDeserializer[BuildTool] {
  override def deserialize(p: JsonParser, ctxt: DeserializationContext): BuildTool =
    BuildTool(p.getValueAsString)
}
